...
class Dijkstra
{
  const std::vector<std::vector<std::pair<int, long long int>>> &graph;
  ...
  void bfs() {
    while (!pq.empty()) {
      ...
      for (const auto &edge:graph[curr.id]) {
        ...
        if (dist[next]>cost+dist[curr.id]) {
          dist[next]=cost+dist[curr.id];
...
public:
  Dijkstra(int N, int A, const std::vector<std::vector<std::pair<int, long long int>>> &graph): graph(graph) {
    dist.assign(N+1, (~0uLL)>>2);
    dist[A]=0;
    pq.push(Vertex(A, 0));
...
int main() 
{
  ...
  int N, M, A, B;  read(N), read(M), read(A), read(B);
  int a, b; long long int c;
  std::vector<std::vector<std::pair<int, long long int>>> graph(N+1);
  for (int i=0;i<M;++i) {
  ...
  Dijkstra fromA(N, A, graph), fromB(N, B, graph);
  std::vector<long long int> distA=fromA.result(), distB=fromB.result();

  assert(distA[B]==distB[A]);
  long long int l=distA[B];

  std::vector<int> path;
  for (int i=1;i<=N;++i) {
    if (distA[i]+distB[i]==l) path.push_back(i);
  }
  writeln(path.size());
  for (int &x:path) write(x), write(' ');
