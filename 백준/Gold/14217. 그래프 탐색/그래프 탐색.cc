...
#endif
void BFS(const std::vector<std::unordered_set<int>> &graph, std::vector<int> &dist) 
{
  dist[0]=0;
  std::queue<int> q;
  q.push(0);
  std::vector<bool> visited(dist.size(), 0);
  visited[0]=1;
...
int main() {
  ...
  for (int i=0;i<q;++i) {
    ...
    BFS(graph, dist);
    for (const int &d:dist) write(d), write(' ');
    write('\n');
