...
int main() {
  ...
  std::vector<int> enter_x(1'000'000, 0), exit_x(1'000'000, 0);
  std::vector<int> enter_y(1'000'000, 0), exit_y(1'000'000, 0);
  for (int i=0;i<n;++i) {
    read(x1), read(y1), read(x2), read(y2), read(x3), read(y3);
    ++enter_x[std::min(x1, std::min(x2, x3))];
    ++exit_x[std::max(x1, std::max(x2, x3))];
    ++enter_y[std::min(y1, std::min(y2, y3))];
    ++exit_y[std::max(y1, std::max(y2, y3))];
  }

  std::vector<int> cnt_x(1'000'000, 0), cnt_y(1'000'000, 0);
  ...
  for (int i=0;i<m;++i) {
    readline(line, 11);
    if (line[0]=='x') writeln(cnt_x[std::stoi(line.substr(4))]);
    else writeln(cnt_y[std::stoi(line.substr(4))]);
  }
