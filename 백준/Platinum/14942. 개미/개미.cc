...
int main() 
{
  ...
  while (!q.empty()) {
    int curr=q.front();
    q.pop();
    for (const auto &[next, dist]:graph[curr]) {
  ...
  std::vector<std::vector<int>> sparse_table(n, std::vector<int>(18, -1));
  for (int i=1;i<n;++i) {
    sparse_table[i][0]=parent[i];
  }
  for (int k=1;k<18;++k) for (int i=0;i<n;++i) {
    if (sparse_table[i][k-1]!=-1) {
      int mid=sparse_table[i][k-1];
      sparse_table[i][k]=sparse_table[mid][k-1];
    }
  }

  for (int i=0;i<n;++i) {
    if (e[i]>=d[i]) writeln('1');
    else {
      int energy=e[i];
      int curr=i;
      for (int k=17;k>=0;--k) {
        int j=sparse_table[curr][k];
        if (j!=-1 && d[curr]-d[j]<=energy) {
          energy-=d[curr]-d[j];
          curr=j;
        } 
      }
