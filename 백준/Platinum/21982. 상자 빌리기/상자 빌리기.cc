...
int main() 
{
  ...
  while (t--) {
    read(n), read(x);
    long long int hs, ha, hb, hc;
    read(hs), read(ha), read(hb), read(hc);
    long long int ws, wa, wb, wc;
    read(ws), read(wa), read(wb), read(wc);

    std::vector<long long int> h(n), w(n), v(n);
    h[0]=hs%hc+1;
    w[0]=ws%wc+1;
    v[0]=h[0]*w[0];
    for (int i=1;i<n;++i) {
    ...
    for (int i=1;i<n;++i) {
      while (!dq.empty() && h[dq.front()]<h[i]-x) dq.pop_front();
      if (!dq.empty()) max=std::max(max, v[dq.front()]+v[i]);
