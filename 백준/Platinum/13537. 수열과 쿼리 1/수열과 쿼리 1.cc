...
private:
  ...
  int _query(int node, int start, int end, const int &i, const int &j,
             const int &k) {
    if (i > end || j < start)
      return 0;
    if (i <= start && end <= j) {
      return (tree[node].end() -
              std::upper_bound(tree[node].begin(), tree[node].end(), k));
    }
    int mid = (start + end) / 2;
    return _query(node * 2, start, mid, i, j, k) +
           _query(node * 2 + 1, mid + 1, end, i, j, k);
  }
...
public:
  ...
  int query(int i, int j, int k) { return _query(1, 0, n - 1, i, j, k); }
};
